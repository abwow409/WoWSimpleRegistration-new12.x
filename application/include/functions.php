...
function validate_recaptcha()
{
    ...
    try {
        $verify = curl_init();
        curl_setopt($verify, CURLOPT_URL, "https://recaptcha.net/recaptcha/api/siteverify?secret=" . get_config('captcha_secret') . "&response=" . $_POST['g-recaptcha-response']);
        curl_setopt($verify, CURLOPT_RETURNTRANSFER, true);
        $response = curl_exec($verify);
...
function getCaptchaJS()
{
    if (!empty(get_config('captcha_type'))) {
        if (get_config('captcha_type') == 1) {
            ...
            return '<script src="https://recaptcha.net/recaptcha/api.js?hl=' . get_config('captcha_language') . '" async defer></script><style>.g-recaptcha { display: inline-block;}</style>';
